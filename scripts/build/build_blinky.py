#!/usr/bin/env python3
"""
Build script for blinky example
...
def main():
    parser = argparse.ArgumentParser(description='Build blinky example')
    ...
    build_dir = project_root / 'builds' / 'build_blinky'
    target = 'blinky'
    
    print_colored(f"\n{'='*80}", Colors.CYAN)
    print_colored(f"Building {target} example", Colors.CYAN)
    print_colored(f"{'='*80}\n", Colors.CYAN)
    ...
    cmake_args = [
        'cmake',
        '-G', 'Ninja',
        '../..',  # Source is two levels up from builds/build_blinky
        '-DPLATFORM=stm32',
